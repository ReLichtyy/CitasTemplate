...
- **Pages are organized by domain under `src/pages/`**, mirroring the backend:
  ...
  - `publico/` — `LandingPage` (generic booking pitch + `/health` check), `EquipoPage` — ungated marketing pages. The public service catalog is not a page of its own: it is the `#servicios` section of `/equipo` (the phone's `BottomNav` "Servicios" tab links there). `EquipoPage` reads the real catalog from `GET /empleados` and `GET /servicios`; there is no hardcoded catalog left in a page. The admin/empleado management view of the same domain stays in `gestion/servicios/ServiciosListPage`.
