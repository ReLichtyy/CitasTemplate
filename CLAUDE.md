...
- Almost every domain service on the backend is still a stub (`NotImplementedException`). The persistence decision is **closed**: MariaDB/MySQL via Prisma, one database per business (no `negocioId` anywhere). The data model lives in `apiBase/prisma/schema.prisma`; the migration has not been run yet.
- **Read `spec/` before implementing anything.** Short, one file per component: `01-modelo-datos`, `02-reservas-concurrencia`, `03-autorizacion`, `04-contrato-api`, `05-marca-y-responsive`. `spec/README.md` holds the closed decisions and the implementation order. When a spec and this file disagree, the spec wins.
